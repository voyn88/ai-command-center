...
def test_plan_fails_closed_when_a_cost_event_is_corrupt(monkeypatch, pool):
    """`daily_spend_usd` raising `SpendUnknownError` (a corrupt cost event or
    an overflowed total, not merely a DB outage) must fail closed identically
    to any other unreadable-spend case — the diagnostic branch changes what
    gets logged, not the fail-closed outcome."""
    _enable_master_switch()
