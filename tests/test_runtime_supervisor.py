...
def test_start_raw_self_heals_a_same_host_crashed_workspace_lock(
    ...
    host: `start_raw` reconciles and retries before raising
...
def test_start_raw_self_heals_a_stale_workspace_lock_across_a_nonzero_grace_window(
    git_repo, configure_project_repo, fake_claude, monkeypatch
):
    """Regression test for the adversarial-review finding on the first attempt
    at this fix: `reconcile()`'s cross-process debounce (audit P0) never
    terminalizes a row on the observation that first notices it looks gone —
    that call only starts the row's grace-window clock (see
    `_RECONCILE_ABSENCE_GRACE_SECONDS`), so a peer genuinely mid-launch is
    never yanked out from under itself. A caller that reconciled exactly once
    and retried immediately raced that same clock and lost every time in
    production, even though the equivalent test always passed — because
    `tests/conftest.py`'s autouse `_immediate_reconcile` fixture defaults the
    grace window to `0.0` for the whole suite, which makes a single
    `reconcile()` call terminalize immediately and hides the bug. This test
    restores a real, nonzero grace window on this one `Supervisor` (the way
    `tests/test_runtime_reconciliation.py` already does for `reconcile()`
    itself) so self-heal is proven against the actual production timing, not
    the suite-wide shortcut."""
    configure_project_repo("AIOS", git_repo)
    sup = supervisor.Supervisor()
    monkeypatch.setattr(sup, "_reconcile_absence_grace", 0.2)
...
def test_start_raw_still_raises_workspace_locked_error_when_conflict_is_genuinely_alive(
    ...
    it is never treated as a self-heal candidate and the retry loses fast —
    proving this does not degrade into a loop that eventually forces the
    launch through against a live peer."""
    fake_claude["FAKE_CLAUDE_EXTRA_SLEEP"] = "5"
    configure_project_repo("AIOS", git_repo)
    sup = supervisor.Supervisor()
    first = sup.start_raw(
    ...
    try:
        ...
        assert excinfo.value.conflicting_run["id"] == first["id"]
        # A genuinely alive conflict is never a self-heal candidate, so this
        # must fail fast, not wait out the grace window.
