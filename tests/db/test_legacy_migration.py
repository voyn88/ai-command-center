...
def _clean_report(snapshot) -> dict:
    """A hand-fabricated "clean" report — never produced by an actual `migrate()`
    run. `retire_legacy_authority` must not accept this as authorization; it is
    only used by `test_cutover_refuses_a_fabricated_report_against_an_empty_target`
    to prove that.
    """
    tables = {
        cls.spec.table: {
...
def test_import_invalidates_an_older_green_report_before_target_writes(
    ...
    monkeypatch.setattr(
        "command_center.db.legacy_migration._ordered_mirrors", list
    )
...
def test_cutover_requires_clean_report_and_retires_replaced_sources(
    ...
    snapshot = create_snapshot(data, tmp_path / "rollback")
    report = tmp_path / "report.json"
    migrate(snapshot.directory, admin_conn, report)

    marker = retire_legacy_authority(data, snapshot.directory, admin_conn, report)
...
def test_cutover_refuses_a_fabricated_report_against_an_empty_target(
    tmp_path: Path, admin_conn, pg_connection_factory
) -> None:
    """The exploit the adversarial review found: an operator (or attacker with
    filesystem access) hand-edits a reconciliation report to say ``"clean":
    true`` without ever running `migrate()`. `retire_legacy_authority` must
    never trust that file — it has to reconcile the live target itself before
    retiring anything.
    """
    data = _data_dir(tmp_path)
    snapshot = create_snapshot(data, tmp_path / "rollback")
    ...
    with pytest.raises(MigrationRefused, match="does not reconcile clean"):
        retire_legacy_authority(data, snapshot.directory, admin_conn, report)

    # PostgreSQL was never imported into, so nothing may have been retired.
    assert (data / "runtime.db").stat().st_mode & 0o222 != 0
    assert (data / "execution_queue.json").exists()


def test_cutover_refuses_data_written_after_snapshot(
    tmp_path: Path, admin_conn, pg_connection_factory
) -> None:
    data = _data_dir(tmp_path)
    snapshot = create_snapshot(data, tmp_path / "rollback")
...
def test_cutover_refuses_legacy_file_created_after_snapshot(
    tmp_path: Path, admin_conn, pg_connection_factory
) -> None:
    data = _data_dir(tmp_path)
    snapshot = create_snapshot(data, tmp_path / "rollback")
...
def test_cutover_refuses_sqlite_rows_written_after_snapshot(
    tmp_path: Path, admin_conn, pg_connection_factory
) -> None:
    data = _data_dir(tmp_path)
    snapshot = create_snapshot(data, tmp_path / "rollback")
...
def test_cutover_refuses_when_a_queue_writer_already_holds_the_lock(
    tmp_path: Path,
) -> None:
    """A write in flight when cutover starts must block or fail it, not race it.

    `admin_conn`/PostgreSQL is irrelevant here: the queue lock is acquired
    before anything about the target is touched, so a lock already held is
    enough to prove the check-then-act window is closed without needing a
    real database.
    """
    data = _data_dir(tmp_path)
    snapshot = create_snapshot(data, tmp_path / "rollback")
    ...
    with (
        storage.file_lock(data / QUEUE_LOCK_FILE_NAME, timeout=5),
        pytest.raises(MigrationRefused, match="queue lock"),
    ):
        retire_legacy_authority(
            data, snapshot.directory, None, report, lock_timeout=0.2
        )


def test_cutover_refuses_when_a_legacy_writer_holds_the_sqlite_lock(
    tmp_path: Path,
) -> None:
    """Same race, for `runtime.db` itself: a `BEGIN IMMEDIATE` writer already in
    flight must force cutover to wait or fail, never proceed past it.
    """
    data = _data_dir(tmp_path)
    snapshot = create_snapshot(data, tmp_path / "rollback")
    report = tmp_path / "report.json"

    writer = sqlite3.connect(data / "runtime.db", isolation_level=None)
    try:
        writer.execute("BEGIN IMMEDIATE")
        with pytest.raises(MigrationRefused, match="SQLite write lock"):
            retire_legacy_authority(
                data, snapshot.directory, None, report, lock_timeout=0.2
            )
    finally:
...
def test_retire_refuses_if_the_wal_sidecar_still_has_unmerged_frames(
    tmp_path: Path,
) -> None:
    """A chmod on `runtime.db` alone is not the retirement boundary: an
    already-open connection keeps writing through `-wal` regardless of the
    main file's mode bits. Unmerged bytes left there after the checkpoint are
    exactly the evidence that a legacy writer is still connected.
    """
    db = tmp_path / "runtime.db"
    core.migrate(db)
    (db.with_name(db.name + "-wal")).write_bytes(b"\x00" * 64)

    with pytest.raises(MigrationRefused, match="unmerged writes"):
        _retire_sqlite_database(db)
...
def _cutover(tmp_path: Path, data: Path, connection):
    snapshot = create_snapshot(data, tmp_path / "rollback")
    report = tmp_path / "report.json"
    migrate(snapshot.directory, connection, report)
    retire_legacy_authority(data, snapshot.directory, connection, report)
...
def test_verify_accepts_a_clean_cutover(
    tmp_path: Path, admin_conn, pg_connection_factory
) -> None:
    data = _data_dir(tmp_path)
    _cutover(tmp_path, data, admin_conn)
...
def test_verify_reports_a_queue_a_legacy_writer_re_created(
    tmp_path: Path, admin_conn, pg_connection_factory
) -> None:
    data = _data_dir(tmp_path)
    _cutover(tmp_path, data, admin_conn)
...
def test_verify_reports_a_retired_store_made_writable_again(
    tmp_path: Path, admin_conn, pg_connection_factory
) -> None:
    data = _data_dir(tmp_path)
    _cutover(tmp_path, data, admin_conn)
...
def test_cutover_is_idempotent(
    ...
    data = _data_dir(tmp_path)
    snapshot, report = _cutover(tmp_path, data, admin_conn)
