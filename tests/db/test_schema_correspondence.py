...
CORRESPONDING_MIGRATIONS = (
    ...
    / "db"
    / "sql"
    / "0025_run_completion_insert_seq.up.sql",
)
...
def postgres_schema(admin_conn) -> dict:
    ...
    generation = {
        (table, column): how
        for table, column, how in admin_conn.execute(
    ...
    return {
        ...
        "unique_shapes": unique_shapes,
        "check_shapes": check_shapes,
        "identity_columns": set(generation),
        # `ALWAYS` vs `BY DEFAULT`: same `is_identity`, opposite behaviour on an
        # insert that names the column. See the test that pins it.
...
def test_identity_columns_are_the_documented_nine(postgres_schema) -> None:
    """An identity column changes how the import must be written.

    For the seven `GENERATED ALWAYS` surrogate keys, an explicit `INSERT` of the
    id is rejected without `OVERRIDING SYSTEM VALUE`, and the sequence has to be
    restarted past `max(id)` afterwards or the first ordinary insert collides on
    the primary key.

    `run.insert_seq` and `completion.insert_seq` (`0025_run_completion_insert_seq`)
    are the first identity columns here that are **not** keys and are
    `GENERATED BY DEFAULT` rather than `ALWAYS` — precisely so the explicit value
    the mirror carries is accepted without `OVERRIDING SYSTEM VALUE`. They keep
    the second obligation and drop the first, which is exactly the distinction
    this pin exists to keep visible: `is_identity` alone does not say which.
    Neither obligation shows up in a type table, so the set is pinned here.
    """
    assert postgres_schema["identity_columns"] == {
        ...
        ("model_event", "id"),
        ("run", "insert_seq"),
        ("completion", "insert_seq"),
    }


def test_the_non_key_identity_columns_are_generated_by_default(postgres_schema) -> None:
    """The half of the distinction above that `is_identity` cannot express.

    `GENERATED ALWAYS` would refuse every write the mirror makes — silently,
    because the dual-write hook logs and moves on — so this is the difference
    between a mirrored column and an empty one.
    """
    assert postgres_schema["identity_generation"] == {
        ("run_event", "id"): "ALWAYS",
        ("completion_event", "id"): "ALWAYS",
        ("completion_validation", "id"): "ALWAYS",
        ("proposal_event", "id"): "ALWAYS",
        ("proposal_evidence", "id"): "ALWAYS",
        ("council_event", "id"): "ALWAYS",
        ("model_event", "id"): "ALWAYS",
        ("run", "insert_seq"): "BY DEFAULT",
        ("completion", "insert_seq"): "BY DEFAULT",
    }
...
def test_the_columns_needing_value_conversion_are_the_documented_ones(
    ...
    # 402 before `0025_run_completion_insert_seq` added `insert_seq` to `run` and
    # to `completion`. Neither is a conversion: `INTEGER` -> `bigint` is the one
    # pairing in this schema that moves nothing.
    assert sum(
