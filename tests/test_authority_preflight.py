...
def test_detects_sudo_with_verbs_outside_the_old_allowlist():
    """Regression for the PR #530 rejection: a verb *allowlist* (the
    original approach) will always miss some real admin command -- `cp`,
    `rm`, `bash -c`, `python` are exactly the shapes that slipped through
    and reached the executor cascade unblocked."""
    assert ap.detected_authority("sudo cp /etc/shadow /tmp/x") == frozenset({ap.AUTHORITY_ROOT})
    assert ap.detected_authority("sudo rm -rf /var/lib/postgresql/data") == frozenset({ap.AUTHORITY_ROOT})
    assert ap.detected_authority("sudo bash -c 'echo hi > /root/x'") == frozenset({ap.AUTHORITY_ROOT})
    assert ap.detected_authority("sudo python scripts/rotate_keys.py") == frozenset({ap.AUTHORITY_ROOT})


def test_does_not_misfire_on_a_bare_dash_u_flag_unrelated_to_postgres():
    """Regression for the PR #530 rejection: `-u postgres` matched
    context-free, so `docker run -u postgres ...` (docker's `-u` sets the
    *container* user, not a host PostgreSQL role) was falsely parked. The
    flag must sit inside an actual role-switching command (`sudo -u`, `su`,
    or a PostgreSQL client binary) to count."""
    assert ap.detected_authority("docker run -u postgres myimage") == frozenset()
    assert ap.detected_authority("useradd -u postgres appuser") == frozenset()
