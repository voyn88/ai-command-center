...
def _backlog_tasks(
    ...
    `rich` is always `[]` for a file generated by `backlog-export` (BO-S4,
    VOYN-W0-BACKLOG-ORCHESTRATOR) — it renders only `VOYN_RECOMMENDATION`
    lines, never the bold task lines this parses — so every task falls back
