...
What this repository owns is the part only it can: the schema declared in
`roles.ALL_TABLES` and its migrations, the `aicc_*` roles and their grants, the
repositories, the backup/restore policy, and the composition that decides what
"ready" means for this service. AIOS Core knows none of those.
