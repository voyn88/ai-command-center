...
the queue's own lease mechanism; it is `agent_runner.MIN_TIMEOUT_SECONDS` / `MAX_TIMEOUT_SECONDS`
verbatim — the existing hard cap on how long a single `run_claude_code` invocation may run, which
`resolve_timeout` and `timeout_for_task` already clamp every non-worker caller to. The payload
contract mirrors that cap at parse time, where the operator can see *why* a request was refused,
...
- `payloads.py`'s timeout bound is pinned to `agent_runner`'s cap by citation, not by a separately
  maintained constant; a future change to `agent_runner.MAX_TIMEOUT_SECONDS` should be accompanied by
  a look at whether `_MAX_TIMEOUT_SECONDS` here should move with it.
...
- `command_center/worker/payloads.py` — the `agent_run` payload contract, `_MIN_TIMEOUT_SECONDS` /
