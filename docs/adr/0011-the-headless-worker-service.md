...
id shape) are refusals, not silent clamps: `_MAX_TIMEOUT_SECONDS` caps a payload's requested
timeout at 3600s — the agent-run ceiling the `@` unit's `TimeoutStopSec` (3660s, below) is sized
for — not the queue-visibility window, which defaults to 300s but is irrelevant to how long a run
may take, since the heartbeat renews it continuously for as long as the handler stays alive. A
payload requesting more than that ceiling is refused at parse time because nothing in the deployed
fleet is provisioned to let the agent subprocess run that long, not because it would outrun a
lease that the daemon keeps alive on its own.
