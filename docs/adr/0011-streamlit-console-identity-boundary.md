# ADR-0011: the Streamlit console stays local-only; remote reachability is gated on the AIOS-identity boundary, never a bare bind flip
...
elsewhere in this repository far better than a Tornado-backed Streamlit script does — but that is a
product-roadmap fact, not a substitute for this gate applying to Streamlit *while Streamlit exists
and is reachable*.
...
- A future PR that widens Streamlit's bind beyond loopback for a real deployment need must land the
  identity-aware reverse proxy described above in the same change, or it does not pass review. This
  ADR is the standing justification a reviewer cites when rejecting a bind-only widening.
- `tests/test_deployment_exposure.py` continues to gate the default; it is not itself sufficient
  ...
  an operator-chosen non-default carries authentication.
