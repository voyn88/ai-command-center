...
| `data/portfolio_config.json` (+lock) | Portfolio project-id -> local repository path mapping (independent namespace from `project_config.json`'s Kanban projects). Local, machine-specific, gitignored — mirrors `project_config.json`'s own convention. | `portfolio_config.py` | No tracked example (gitignored, no built-in defaults beyond `AICC`/`PRODUCT`); operator re-entry per project id. |
