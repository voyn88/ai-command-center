...
| `data/agent_attestation.json` (+lock) | Per-agent certification evidence (VOYN-AGT-ATTEST): certification test-case results and historical critical-incident counts. `dispatch.attestation.evaluate_attestation` is the pure decision; `dispatch.service.certified_executor_ids` gates `Critical`-priority dispatch (the "critical zone") to only certified agents, fail-closed — an agent absent from the store is uncertified. | `command_center/dispatch/attestation_config.py` | An agent missing/lost from the store is simply uncertified again (safe default); re-recorded by an operator/CI job. |
