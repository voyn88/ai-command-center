...
* `identity` — whether the primary *key* is `GENERATED ALWAYS AS IDENTITY`,
  which changes the statement (`OVERRIDING SYSTEM VALUE`) and unlocks
  `resync_identity()`. A non-key identity column (`run.insert_seq`, added
  `GENERATED BY DEFAULT` so the mirror's explicit values are accepted without
  it) needs no declaration at all — it changes nothing about the statement —
  but still takes `resync_identity("insert_seq")` at cutover.
...
class PostgresTableMirror:
    ...
    def resync_identity(self, column: str = "id") -> int:
        """Advance an identity sequence past the largest mirrored value.

        A cutover step, not a write-path one, and the reason it must exist was
        measured rather than assumed: inserts carrying an explicit value leave
        the sequence untouched, so the first row PostgreSQL generates for
        itself starts at 1 and collides with a mirrored row.

        Nothing in a dual-write triggers this — while SQLite is the authority
        every value arrives from the mirror and the sequence is never consulted
        ...
        `column` defaults to `id`, which is what every caller before
        `0025_run_completion_insert_seq` needed: an identity *primary key*,
        declared by `spec.identity`. That migration added an identity column
        that is not a key — `run.insert_seq` and `completion.insert_seq`, whose
        keys are text — and those need the same cutover step for the same
        reason, so the column is a parameter rather than a literal. Which
        columns actually have a sequence is asked of the database
        (`pg_get_serial_sequence` returns NULL when there is none) instead of
        being re-declared here, because a second declaration of a fact the
        schema already states is a second thing that can be wrong.
        """
        spec = self.spec
        if column not in spec.columns:
            raise ValueError(f"{spec.table} has no column {column!r}")
        if column == "id" and not spec.identity:
            raise TypeError(f"{spec.table} has no identity column to resync")
        with self._connection() as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT pg_get_serial_sequence(%s, %s)", (spec.table, column))
                sequence = cur.fetchone()[0]
                if sequence is None:
                    raise TypeError(
                        f"{spec.table}.{column} is not an identity column; nothing to resync"
                    )
                ...
                cur.execute(
                    f"SELECT setval(%s, (SELECT COALESCE(MAX({column}), 1) FROM {spec.table}),"
