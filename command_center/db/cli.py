...
def build_parser() -> argparse.ArgumentParser:
    ...
    exp = sub.add_parser(
        "backlog-export",
        help="Regenerate the Markdown backlog projection from the structured "
        "store (BO-S4): a read projection for the owner, written atomically "
        "so a concurrent importer or reader never sees a partial file.",
    )
    ...
    try:
        with pool.connection() as conn:
            ...
            if args.command == "backlog-export":
                ...
                count = BacklogStore(lambda: nullcontext(conn)).write_projection(
                    args.path
                )
