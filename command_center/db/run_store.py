...
`0004_run_finalized_at`), one `bigint` identity (`insert_seq`, from
`0025_run_completion_insert_seq`), two foreign keys (slice 5). The machinery
carries all of it; what this slice is actually about is the write path.
...
RUN_COLUMNS: tuple[str, ...] = (
    ...
    "finalized_at",
    # Added by `0025_run_completion_insert_seq`, same rule. Monotonic insertion
    # order — see that file for why `created_at` could not serve and why the
    # column is `GENERATED BY DEFAULT`, not `GENERATED ALWAYS`.
    "insert_seq",
)
