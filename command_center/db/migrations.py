...
The *schema* is AICC's: every table in `roles.ALL_TABLES`, their indexes, the
`aicc_*` grants that go with them, and the plain SQL files in `sql/` named
`NNNN_slug.up.sql` with a matching `NNNN_slug.down.sql`. AIOS Core knows none
of that and must not. `roles.ALL_TABLES` is the count, not a number pinned
here — one changes with every migration, the other used to and was already
wrong (see `tests/db/test_mirror_coverage.py`).
