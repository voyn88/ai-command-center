...
class WorkQueueReadStore:
    ...
    def queue_metrics(self, *, queue: str | None = None) -> list[dict[str, Any]]:
        ...
        healthy steady state, not a missing measurement. A ``ready`` item can
        carry a future ``available_at`` (delayed enqueue, or backoff after a
        retryable failure — see ``queue_enqueue``/``queue_fail`` in
        ``0002_queue_claim.up.sql``); such an item is not yet claimable, so it
        does not count as backlog and is excluded the same way
        ``queue_claim``'s own predicate excludes it (``available_at <=
        now()``). Without that filter a queue holding only delayed work would
        ...
        sql = (
            ...
            "    FILTER (WHERE i.state = 'ready' AND i.available_at <= now()"
