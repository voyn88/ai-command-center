DROP FUNCTION IF EXISTS backlog_clear_evidence(text, text, text, text);
DROP FUNCTION IF EXISTS backlog_park_unroutable(text, text, integer);

-- Restore 0011's backlog_ingest_results (no malformed-pr-url shape gate).
