...
--
-- `backlog_ingest_results` has the identical hazard on its OWN lease
-- release, and it is not hypothetical here: `backlog_dispatch`'s WIP-limit
-- query counts LEASES the planner holds, not tasks, precisely because one
-- planner may hold a single `repo:X` lease across more than one dispatched
-- task in that repo (the lease renews rather than duplicates). A prior
-- version of this migration released `repo:` || the ingested task's repo
-- unconditionally at the end of every loop iteration — correct only when
-- that task was the sole IN_PROGRESS occupant of its repo. When a SECOND
-- task shares the repo and is still running (no terminal work item yet, so
-- it never enters this loop at all), that unconditional release stripped
-- the lease out from under it and let a second writer into the same
-- checkout. The fix mirrors dispatch's: before releasing, check for any
-- OTHER `backlog_task` row on this repo still `IN_PROGRESS` (the row this
-- iteration is about is excluded by id — it has already been moved off
-- IN_PROGRESS above, or, on a refusal, deliberately left exactly where it
-- was). A survivor means the lease is still spoken for, and the release is
...
DECLARE r record; t backlog_task%ROWTYPE; tv backlog_verdict; rv backlog_verdict;
        ...
        v_lease_needed_elsewhere boolean;
BEGIN
    FOR r IN
    ...
    LOOP
        ...
        IF r.q_state = 'succeeded' AND v_task_status = 'completed' AND v_pr IS NOT NULL THEN
            ...
            -- 0011. The order was free while a refusal raised, because the
            -- abort discarded the evidence rows along with everything else.
            -- Now that the refusal commits, recording evidence first would
            -- leave `pr`/`sha` rows on a task this function just declined to
        ...
        -- The lease is repo-scoped, not task-scoped. `backlog_lease_acquire`
        -- renews rather than duplicates when the planner already holds
        -- `repo:X` (see `backlog_dispatch`'s WIP-limit query above, which
        -- counts LEASES, not tasks) -- so two tasks the SAME planner
        -- dispatched into the same repo can share ONE lease row. Releasing
        -- it here unconditionally, as this function used to and as the
        -- unused `backlog_release_terminal` (0006) still does, would strip
        -- that shared lease out from under a sibling task in this repo that
        -- is still running (its work item is not terminal, so it never
        -- enters this loop and never gets a say) and hand the checkout to a
        -- second writer. `r.t_id` is excluded by id rather than relied on to
        -- already read as non-IN_PROGRESS: on a refusal above it is
        -- deliberately left exactly where it was.
        SELECT EXISTS (
            SELECT 1 FROM backlog_task bt
             WHERE bt.repo = r.repo AND bt.status = 'IN_PROGRESS'
               AND bt.task_id <> r.t_id
        ) INTO v_lease_needed_elsewhere;

        IF v_lease_needed_elsewhere THEN
            lv.ok := false; lv.reason := 'retained_for_sibling_task';
        ELSE
