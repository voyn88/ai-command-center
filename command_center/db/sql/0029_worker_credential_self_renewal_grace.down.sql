...
-- equals the ledger expiry) and the 0013 one-argument expiry query stay; the
-- grace knob and both graced overloads go.
--
-- Fidelity: 0003 is the ONLY definition of identity_assert(text),
-- identity_issue_db_credential and enroll_rotate_self before 0029 (verified
-- against the migration set when this file was written), so the bodies below
-- are their immediately prior definitions. They are restored with
