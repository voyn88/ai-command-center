...
-- to the operator deciding whether to redrive. BOTH no-fault families get
-- the rule: `queue_fail_lease_wait` (0022) and `queue_fail_infra_wait`
-- (0024), whose dead-letter branch had the same stray increment.
--
-- The refund is written INTO the UPDATE (`attempt_count = greatest(
-- work_item.attempt_count - 1, 0)`, the stored value returned for the audit),
-- never computed from a row read moments earlier: a SELECT-then-UPDATE
-- refund silently overwrites any concurrent change to `attempt_count` between
-- the two statements, and an overwrite there is a lost increment -- an
-- attempt a different delivery spent, handed back (review of 87ccf982).
...
BEGIN
    ...
    -- numbers taken from separate places). The decrement is atomic in the
    -- UPDATE; `v_refunded` is what the row holds afterwards.
    IF v_waits > v_cap THEN
        ...
        UPDATE work_item
           SET state = 'dead', current_attempt_id = NULL,
               attempt_count = greatest(work_item.attempt_count - 1, 0),
         ...
         RETURNING attempt_count INTO v_refunded;
...
-- The capacity-outage family, same rule, same atomic refund. 0024 refunded on
-- its requeue branch (from a row read earlier) and kept the last delivery on
-- its dead-letter branch.
CREATE OR REPLACE FUNCTION queue_fail_infra_wait(
    p_attempt_id text, p_claim_token text, p_reason text,
