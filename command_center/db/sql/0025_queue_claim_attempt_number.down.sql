-- Restore 0002's `queue_claim`, which derives `attempt_no` from the attempt
-- budget. Going back reinstates the head-of-line poisoning described in the
-- up-migration: any item that has been through `queue_fail_lease_wait` becomes
-- unclaimable, and because `queue_claim` takes the oldest due row, so does the
-- queue behind it. The down path exists because every migration here has one,
-- not because this one is safe to run on a fleet using the lease-wait exit.
