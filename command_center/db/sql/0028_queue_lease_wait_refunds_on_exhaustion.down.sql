-- Restore the 0022 lease-wait failure and the 0024 infra-wait failure: the
-- requeue branch refunds the attempt the claim spent, the dead-letter branch
-- keeps it. Down migrations restore the
...
-- 0024's `queue_fail_infra_wait`, verbatim (its only prior definition).
