...
Originally lived in `tests/db/mirror_discovery.py`, written for two suites that
needed the same answer: the mirror contract, which enrols every mirror in its
per-table checks, and the stored-reader fitness gate. `VOYN-W0-AICC-SRV-07`
adds a third caller that is not a test at all — the legacy importer needs the
same table set, in the same dependency order, to run a real backfill — and a
production module importing from `tests/` is backwards. This is that answer,
moved to where a non-test caller can reach it; `tests/db/mirror_discovery.py`
re-exports it so the existing suites do not need to change their imports.

Slice 9's acceptance showed what a second copy of this rule costs. A mirror
declared in `command_center/db/run_mirror.py` with a deliberately wrong key was
collected by neither of the two hand-written copies, passed everything, and the
contract still reported seventeen tables — the blocking defect of that slice,
relocated by renaming a file. Fixing one copy would have left the other, which
is the argument for this module existing at all: a rule with two
implementations has two answers eventually.
...
the only ones still running. The same property matters here for a new reason:
`command_center.db.__init__` promises that importing the package pulls in
neither `aios_db` nor `psycopg`, and this module keeps that promise too.
"""
