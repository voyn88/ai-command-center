...
# module's own docstring). A flag (`-u`, `-i`, ...) or an absolute path is an
# unambiguous command signal on its own. Anything else -- a bare word right
# after `sudo` -- is a command UNLESS that word is one of a small set of
# English words that only ever continue a sentence ("sudo and postgres
# access...", "sudo requires elevated..."). This is a denylist, not an
# allowlist of admin verbs on purpose: an allowlist of verbs (the previous
# approach) misses `sudo cp`, `sudo rm`, `sudo bash -c`, `sudo python ...`
# forever, one missing verb at a time. A denylist of sentence-continuation
# words is the smaller, more stable list -- every executable name defaults to
# "this is a command".
_SUDO_TOKEN = re.compile(r"\bsudo\s+(-\S+(?:\s+\S+)?|/\S+|[^\s.,;:!?]+)")
_SUDO_PROSE_FOLLOWERS = frozenset(
    {
        "and", "or", "but", "so", "if", "when", "while", "since", "because",
        "is", "are", "was", "were", "be", "been", "being",
        "to", "for", "of", "in", "on", "with", "without", "the", "a", "an",
        "access", "privilege", "privileges", "permission", "permissions",
        "boundary", "boundaries", "policy", "policies", "rule", "rules",
        "config", "configuration", "here", "there", "worker", "workers",
        "user", "users", "file", "files", "would", "should", "must", "can",
        "could", "cannot", "not", "never", "always", "usage", "command",
        "commands", "invocation", "detection", "detector", "detects",
        "matches", "match", "matching", "handling", "itself", "rights",
        "grants", "grant", "granting", "gives", "give", "giving", "allows",
        "allow", "allowing", "needs", "need", "needing", "means", "requires",
        "require", "requiring",
    }
)
...
# A PostgreSQL role switch shaped like an actual command. A bare `-u
# postgres` / `-U postgres` anywhere in the text is NOT enough -- plenty of
# tools that have nothing to do with a host PostgreSQL role reuse `-u` for
# something else (`docker run -u postgres ...` sets the *container* user).
# Require the flag to sit inside a command that actually switches to the
# postgres role: `sudo -u postgres`, `su [-] postgres`, or a PostgreSQL
# client binary (`psql`, `pg_dump`, ...) invoked with `-U postgres`.
_PG_CLIENT_TOOLS = (
    r"(?:psql|pg_dump|pg_dumpall|pg_restore|pg_ctl|createdb|dropdb|"
    r"createuser|dropuser|vacuumdb|reindexdb|clusterdb|pgbench)"
)
_POSTGRES_ROLE_COMMAND = re.compile(
    rf"\bsudo\s+-[uU]\s+postgres\b"
    rf"|\bsu\s+(?:-\s+)?postgres\b"
    rf"|\b{_PG_CLIENT_TOOLS}\b[^\n]*?-[uU]\s+postgres\b"
)


def _sudo_command_present(text: str) -> bool:
    for match in _SUDO_TOKEN.finditer(text):
        token = match.group(1)
        if token.startswith("-") or token.startswith("/"):
            return True
        if token.lower() not in _SUDO_PROSE_FOLLOWERS:
            return True
    return False
...
def detected_authority(text: str) -> frozenset[str]:
    ...
    tags: set[str] = set()
    if _sudo_command_present(text) or _ROOT_PROSE.search(text):
