...
# https://github.com/voyn88/ai-command-center/pull/435). Every route below
# waits on `pg_isready` against the exact endpoint the printed DSN points at
# before declaring success, and prints nothing at all if that probe never
# succeeds.
#
...
# Waits for the daemon's own readiness probe, not the host's -- this works
# whether or not a `pg_isready` client is installed outside the container.
wait_ready() {
    local engine="$1" container="$2" tries=0 max=$((READY_TIMEOUT_S * 2))
    while ((tries < max)); do
        "$engine" exec "$container" pg_isready -U postgres >/dev/null 2>&1 && return 0
        sleep 0.5
        tries=$((tries + 1))
    done
    return 1
...
start_container() {
    ...
    if ! wait_ready "$engine" "$CONTAINER_NAME"; then
