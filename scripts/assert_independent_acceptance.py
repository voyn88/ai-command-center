...
The judgement itself (`evaluate` and its supporting parsing) lives in
`command_center.orchestrator.acceptance_policy`, re-exported below, so that
this CI-side gate and `command_center.orchestrator.merge_gateway` — which must
reach the identical verdict before it will spend its own merge credential —
can never quietly drift apart into two implementations of "was this
...
# Run directly (`python3 scripts/assert_independent_acceptance.py`, as the
# acceptance-gate workflow does), Python puts only this file's own directory
# on `sys.path` — not the repository root `command_center` lives under. Insert
# it explicitly rather than requiring the workflow to `pip install` anything:
# `acceptance_policy` is stdlib-only, so this stays a same-runner, no-setup-step
# script rather than gaining an installation dependency it doesn't need.
