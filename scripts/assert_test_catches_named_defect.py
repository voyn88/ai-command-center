...
def check(node_ids: list[str], base: str, extra_files: list[str]) -> int:
    """Prove every node id, one at a time.

    A single combined pytest invocation reports one aggregate outcome for the
    whole set — "at least one failed" — which a strong test satisfies on
    behalf of a weak one bundled alongside it. Judging the set instead of
    each member is exactly how a test that never exercises the fix could hide
    behind one that does, so each node id gets its own run and must clear the
    bar on its own.
    """
    carried = _carried_files(node_ids, extra_files)

    for node_id in node_ids:
        here = _run(ROOT, [node_id])
        if here.returncode != 0:
            print(
                f"{node_id} does not pass at the current head; a test that "
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            confirmed: list[tuple[str, "subprocess.CompletedProcess[str]"]] = []
            for node_id in node_ids:
                there = _run(tree, [node_id])

                if there.returncode == 0:
                    print(
                        f"with every file but {', '.join(carried)} reverted to {base}, "
                        f"{node_id} still passed:\n"
                    )
                ...
                if not failed:
                    # A pytest error (setup/collection) is not the same claim as a
                    # failed assertion. Treating the two alike is the exact mistake
                    # this tool exists to end: a red exit code that is true for the
                    # wrong reason is no more evidence than a green one that is true
                    # for the wrong reason. The likeliest wrong reason here is a
                    # file the test needs that the fix also touched — `--also`
                    # carries it; if that is not it, the failure below needs to be
                    # read, not trusted.
                    reason = f"{errored} error(s)" if errored else "an unparsed result"
                    print(
                        f"{node_id} fails at {base}, but with no failed assertion "
                        f"— {reason} instead:\n"
                    )
                ...
                confirmed.append((node_id, there))
        finally:
    ...
    for node_id, there in confirmed:
        print(f"{node_id}:")
        print(there.stdout[-2000:])
    return 0
