...
the batch without stalling the others. Both required checks carry a `merge_group` trigger — `ci.yml`
runs its gates against the synthetic queue commit, and `acceptance-gate.yml` resolves every pull
request represented by that commit and requires a live, independent exact-head verdict for each one
(`scripts/assert_independent_acceptance.py`) — without both triggers a queued entry would never report
either check and would time out.
